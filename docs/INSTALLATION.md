...
点击“篡改猴”后会展开下图菜单。“添加新脚本…”在“获取新脚本…”下面；“管理面板”在靠下的位置。首次安装点“添加新脚本…”，更新已有脚本则从“管理面板”进入原脚本。

![真实篡改猴菜单：添加新脚本和管理面板入口](images/real-tampermonkey-menu.png)
...
**粘贴位置看这里：**下图白色的大区域就是代码编辑区。先点击里面，再按 Ctrl+A、Ctrl+V，最后 Ctrl+S。图中是已经粘贴好本项目代码的样子；新建脚本时应替换原有默认代码。

![真实油猴编辑器：在代码区全选替换源码并保存](images/real-tampermonkey-editor.png)

**保存后核对开关：**下图中 3.1.1 这一行左侧开关为绿色，表示已启用；灰色是停用。右侧铅笔图标可重新进入编辑器。截图中保留了早期测试版本，只需启用正在使用的一份，不用照着安装多个版本，也不用为了更新而删除进度数据。

![真实已安装脚本列表：只启用正在使用的版本](images/real-tampermonkey-enabled.png)
...
**完成标志：**页面右上角出现“百度网盘转存与换链助手”，能看到“Excel 导入”和“粘贴分享文本”。上面的菜单截图背景中就是实际助手面板；点击菜单外的空白处收起油猴菜单，即可操作面板。
